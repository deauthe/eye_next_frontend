...
export const followDesigner = async (userId, designerId) => {
  try {
    const response = await fetch(`${API_URL}/api/user/follow`, {
      ...
      body: JSON.stringify({ userId, designerId }),
    });

    if (!response.ok) {
      throw new Error("Failed to follow designer");
    }
...
export const getRandomDesigner = async ({ designer_id }) => {
  const config = {
    method: "GET",
    headers: {
      "x-api-key": "token",
